...
def main():
    ...
    write_parser.add_argument(
        "note_words", nargs="*", help="Note as consecutive words"
    )

    # Cycle subparser
    cycle_parser = subparsers.add_parser(
        "cycle", help="cycles the engine once"
    )
    cycle_parser.add_argument(
        "-c",
        "--continuous",
        action="store_true",
        help="cycles in a loop until interrupted",
    )
    cycle_parser.add_argument(
        "-a",
        "--all",
        action="store_true",
        help="cycles all notes and reprocess annotations until all are processed",
    )
    cycle_parser.add_argument("-i", "--iterations", type=int)

    # Read subparser with type-specific subcommands
    read_parser = subparsers.add_parser(
        "read", help="Read notes with filters"
    )
    read_subparsers = read_parser.add_subparsers(
        dest="type", help="Type of item to read"
    )
    ...
    note_parser.add_argument(
        "-s", "--search", help="Search for notes containing this text"
    )
    note_parser.add_argument(
        "-l",
        "--limit",
    ...
    todo_parser.add_argument(
        "-s", "--search", help="Search for todos containing this text"
    )
    todo_parser.add_argument(
        "-l",
        "--limit",
    ...
    todo_status_group.add_argument(
        "-i",
        "--incomplete-only",
        action="store_true",
    ...
    # Action subparser
    action_parser = read_subparsers.add_parser(
        "action", help="Read actions"
    )
    add_date_filters(action_parser)
    action_parser.add_argument(
        "-s", "--search", help="Search for actions containing this text"
    )
    action_parser.add_argument(
        "-l",
        "--limit",
    ...
    observation_parser = read_subparsers.add_parser(
        "observation", help="Read observations"
    )
    ...
    # Curiosity subparser
    curiosity_parser = read_subparsers.add_parser(
        "curiosity", help="Read curiosities"
    )
    add_date_filters(curiosity_parser)
    curiosity_parser.add_argument(
        "-s",
        "--search",
    ...
    query_parser = subparsers.add_parser(
        "query", help="get an answer about your data from a query"
    )
    query_parser.add_argument(
        "query_words", nargs="*", help="Prompt for the summary"
    )
    ...
    if args.command == "write":
        ...
        db.Note.create(
            note
        )  # Note: You might want to update this for other types
        print("noted")
    ...
    if args.command == "cycle":
        ...
        if args.type == "todo":
            if args.incomplete_only and args.complete_only:
                parser.error(
                    "Cannot use both --incomplete-only and --complete-only."
                )
            ...
            annotations = db.Curiosity.get_all(
                before=args.before,
                after=args.after,
                search=args.search,
                limit=args.limit,
            ...
            notes.reverse()
            pretty_notes = rendering.strf_notes(
                notes, show_processed_text=True
            )
            print(pretty_notes)
