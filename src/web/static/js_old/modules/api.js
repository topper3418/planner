...
export async function fetchTabContent() {
  ...
  try {
    const endpoint =
      currentTab === "curiosities" ? "/api/curiosities" : `/api/${currentTab}`;
    ...
    if (filterValues.startTime)
      params.append(
        "startTime",
        formatDateTimeFromUTC(filterValues.startTime, true),
      );
    if (filterValues.endTime)
      params.append(
        "endTime",
        formatDateTimeFromUTC(filterValues.endTime, true),
      );
    ...
    if (currentTab === "notes") {
      ...
      items = (data.curiosities || [])
        .reverse()
        .map((curiosity) => renderCuriosity(curiosity));
    }
    if (items.length === 0) {
    ...
    document.getElementById("middle-content").textContent =
      `Error: ${error.message}`;
