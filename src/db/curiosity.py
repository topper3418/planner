...
class Curiosity(BaseModel):
    ...
    @classmethod
    def get_by_source_note_id(cls, source_note_id: int) -> List["Curiosity"]:
        """
        Returns a Curiosity object by its source annotation ID.
        """
        with get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM curiosities WHERE source_note_id = ?", (source_note_id,))
    ...
    def save(self):
        ...
        if not self.id:
            ...
            with get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(query, (self.curiosity_text, self.source_note_id))
            ...
            with get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(query, (self.curiosity_text, self.source_note_id, self.id))
                conn.commit()
